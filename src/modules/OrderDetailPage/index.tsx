...
import { AlertCircle, ArrowLeft, CheckCircle, Loader2, Package, Truck, XCircle } from 'lucide-react';
            ...
            <div className="border-gray-200 border-b p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Placed on {format(order?.createdAt || '', 'dd/MM/yyyy')}</p>
                </div>
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                  ...
                  <div className="mt-6 rounded-lg border border-gray-200 p-4">
                    ...
                    <div className="space-y-2">
                      <HStack className="justify-between">
                        ...
                        <span className="font-medium text-sm">{formatNumber(order?.subtotal || 0)}</span>
                      </HStack>
                      <HStack className="justify-between">
                        <span className="text-gray-600 text-sm">Shipping</span>
                        <span className="font-medium text-sm">{formatNumber(order?.shippingFee || 0)}</span>
                      </HStack>
                      {(order?.discount || 0) > 0 && (
                        <HStack className="justify-between">
                          <span className="text-gray-600 text-sm">Discount</span>
                          <span className="font-medium text-red-600 text-sm">-{formatNumber(order?.discount || 0)}</span>
                        </HStack>
                      )}
                      <Separator className="my-2" />
                      <HStack className="justify-between">
                        <span className="font-medium">Total</span>
                        <span className="font-medium">{formatNumber(order?.total || 0)}</span>
                      </HStack>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <div className="rounded-lg border border-gray-200 p-4">
                    ...
                    <VStack spacing={8} align="start">
                      <div>
                        ...
                        <p className="text-gray-600 text-sm">{order?.shippingAddress.phoneNumber}</p>
                      </div>
                      <div>
                  ...
                  <div className="mt-6 rounded-lg border border-gray-200 p-4">
                    ...
                    <p className="text-gray-700 capitalize">{order?.paymentMethod.type.replace('_', ' ')}</p>
                  </div>

                  {order?.notes && (
                    <div className="mt-6 rounded-lg border border-gray-200 p-4">
                      <h3 className="mb-4 font-medium text-lg">Order Notes</h3>
